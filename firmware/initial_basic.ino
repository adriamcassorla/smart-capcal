...
CRGBPalette16 lightPalettes[] = { lightYellowPalette, darkOrangePalette, lightRedPalette, lightGreenPalette };
CRGBPalette16 darkPalettes[] = { lightYellowPalette, darkOrangePalette, darkRedPalette, darkGreenPalette };
...
void chromotherapy(int maxBrightness) {
  applyRandomPalette(ledsInferior, bottomPalette, NUM_LEDS_INFERIOR, 75, maxBrightness * 0.2, maxBrightness);
  applyRandomPalette(ledsLectura, topPalette, NUM_LEDS_LECTURA, 10, maxBrightness * 0.2, maxBrightness * 0.7);
  applyRandomPalette(ledsTop, topPalette, NUM_LEDS_TOP, 100, maxBrightness * 0.2, maxBrightness);
  applyRandomPalette(ledsTren, bottomPalette, NUM_LEDS_TREN, 100, maxBrightness * 0.2, maxBrightness * 0.8);
}
...
void animateNightLight(bool isLeftSide) {
  ...
  for (uint8_t n=90; n<NUM_LEDS_INFERIOR; n++){
    ledsInferior[n] = blend(CRGB::Black, CRGB::DarkGoldenrod, tempNightBrightness);
  }
